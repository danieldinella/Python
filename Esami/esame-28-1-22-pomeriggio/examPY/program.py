...
def ex1(file1, file2, file3):
    ...
    with open(file2) as F2:
        t2 = F2.readlines()
    for i,riga in enumerate(t1):
        riga = riga.replace(',',' ')
        riga = riga.replace(';',' ')
        t1[i] = riga.split()
    for i,riga in enumerate(t2):
        riga = riga.replace(',',' ')
        riga = riga.replace(';',' ')
        t2[i] = riga.split()
    righe = []
    ...
    while ciclo:
        ...
        for r in range(len(m)):
            for c in range(len(m[0])):
                if m[r][c] == 1:
                    if r != 0 and m[r-1][c] == 2:
                        m[r][c] = 3
                        break
                    if r != len(m)-1 and m[r+1][c] == 2:
                        m[r][c] = 3
                        break
                    if c != 0 and m[r][c-1] == 2 :
                        m[r][c] = 3
                        break
                    if c != len(m[0])-1 and m[r][c+1] == 2:
                        m[r][c] = 3
                        break
        for r in range(len(m)):
            for c in range(len(m[0])):
