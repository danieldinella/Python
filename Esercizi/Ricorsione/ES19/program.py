...
def es66(tree):
    ...
    mas,mino = calcola_valori(tree,0,0,0)
    return mas-mino

def calcola_valori(tree,mas,mino,livello):
    if livello>mas:
        mas = livello
    if livello<mino:
        mino = livello
    if tree.sx == None and tree.dx == None:
        ...
        if tree.sx != None:
            mas,mino = calcola_valori(tree.sx, mas, mino, livello-1)
        if tree.dx != None:
            mas,mino = calcola_valori(tree.dx, mas, mino, livello+1)
        return mas,mino
